...
def add_bucket_column(df, n, k_min, k_max, random_state=None):
    """
    ...
    # Start by assigning k_min rows to each bucket
    ...
    while remaining > 0:
        # Find buckets that can still accept more rows
        idxs = np.where(sizes + increments < k_max)[0]
        # Randomly select one such bucket
        idx = rng.choice(idxs)
        increments[idx] += 1
        remaining -= 1
    ...
    # Shuffle the DataFrame to randomize row assignment
    shuffled_df = df.sample(frac=1, random_state=random_state).reset_index(drop=True)

    # Create the bucket labels for each row
    bucket_labels = np.repeat(np.arange(n), final_sizes)
    shuffled_df["bucket"] = bucket_labels

    # Optionally, restore original order (not done here)
    return shuffled_df


def assign_postal_buckets(
...
def assign_groups(
    df: pd.DataFrame, code_col: str = "kod", min_size: int = 10, max_size: int = 16
) -> pd.DataFrame:
    """
    ...
    group_sizes = work.groupby("group_id").transform("count")[code_col]
